...
details_page <- pageCreator(
  tags$a(class = "btn btn-default", href = "/#!/search", "Back to search")
)

about_page <- pageCreator(
  withTags({
  div(class = "center w-60-ns",
    div(class = "row align-items-center mx-0",
      div(class = "col-2 px-0",
        img(class = "w-100", src="https://s3.amazonaws.com/onepercent-production-member-logos/communities-for-a-better-environment/1568175015594.jpeg"),
      ),
      div(class = "col-10",
...
# Routing
router <- make_router(
  default = route("/", other_page),
  route("/", other_page),
  route("other", root_page),
...
ui <- shinyUI(
  fluidPage(class = "p-0 m-0 h-100",
    bootstrap("cosmo"),
    tags$link(rel = "stylesheet", href = "https://unpkg.com/tachyons@4.12.0/css/tachyons.min.css"),
    tags$script(HTML("$('head').append('<meta name=\"viewport\" content=\"width=device-width, initial-scale=1, shrink-to-fit=no\">');$('html, body').css('height', '100%');")),
